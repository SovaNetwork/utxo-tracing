        ...
        for (tx_index, tx) in block.txdata.iter().enumerate() {
            ...
            for input in tx.input.iter() {
                ...
                if let Some(utxo) = self.get_tracked_utxo(&txid_str, vout_index).await {
                    if let Ok(addr) = Address::from_str(&utxo.address) {
                ...
                {
                    if let Ok(addr) = Address::from_script(&txout.script_pub_key, self.network) {
                        address_opt = Some(addr);
                        amount_sat = txout.value.to_sat() as i64;
                        script_hex = hex::encode(txout.script_pub_key.as_bytes());
                        script_type = determine_script_type(txout.script_pub_key.clone());
                    }
